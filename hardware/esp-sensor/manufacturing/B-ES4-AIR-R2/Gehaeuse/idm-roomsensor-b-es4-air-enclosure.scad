...
part = "assembly";
$fn = 48;
revision = is_undef(case_revision_override) ? "B-ES4-AIR" : case_revision_override;
relay_output = is_undef(relay_output_override) ? false : relay_output_override;
...
module connector_side_cuts(z0,height) {
    ...
    if (relay_output) south_cut(122,22,z0,height);
...
module lid_access_openings() {
    ...
    for (p=relay_output ? [[70,77],[77,77],[87,20]] : [[70,77],[77,77],[108,15]])
    ...
    if (relay_output)
        translate([112.5,3,lid_h-roof_t-0.1]) cube([19,14,roof_t+0.3]);
...
module lid_labels() {
    ...
    translate([75,57,lid_h-0.08]) linear_extrude(height=0.53)
        text(str(revision," | xerolux.de | 2026"),size=3.0,halign="center",valign="center");
...
module preview_board() {
    ...
    color("LimeGreen",0.9) {
        ...
        if (relay_output) translate([112.5,5.5,board_z+pcb_t]) cube([19.2,9.2,14.4]);
    }
    ...
    if (relay_output) color("RoyalBlue",0.9) translate([103,19.5,board_z+pcb_t]) cube([10,6.5,6.5]);
}
